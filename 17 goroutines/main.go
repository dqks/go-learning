...
func main() {
	...
	// -------------------------------------------------------------------
	// Мьютекс позволяет блокировать выполнение горутины и возобновлять ее выполнение
	fmt.Println()
	fmt.Println()

	// Тип у мьютекса sync.Mutex
	var mutex sync.Mutex
	var wg sync.WaitGroup

	wg.Add(4)
	fmt.Println("Начало горутин")

	for i := 1; i <= 4; i++ {
		// Передаем ссылку на мьютекс
		go printNumbers(&mutex, &wg)
	}

	wg.Wait()
	fmt.Println("Окончание горутин")
	// Также у sync есть RWMutex, предназначенный для отдельной блокировки/разблокировки
	// Чтения и записи
}

var counter = 0

func printNumbers(m *sync.Mutex, wg *sync.WaitGroup) {
	// Самая первая горутина, которая до сюда дойдет
	// Заблокирует выполнение остальных горутин
	m.Lock()
	counter++
	for i := 0; i <= 5; i++ {
		fmt.Printf("%d Goroutine -, %d\n", counter, i)
	}
	// До тех пор, пока заблокировавшая горутина не дойдет до сюда
	// И не разблокирует мьютекс
	m.Unlock()
	wg.Done()
}
